function setup_e_waybill_actions(doctype) {
    ...
    frappe.ui.form.on(doctype, {
        ...
        async on_submit(frm) {
            ...
            await frappe.xcall(
                "gst_india.gst_india.utils.e_waybill.generate_e_waybill",
...
function fetch_e_waybill_data(frm, args, callback) {
    ...
    frappe.call({
        method: "gst_india.gst_india.utils.e_waybill.fetch_e_waybill_data",
...
function show_generate_e_waybill_dialog(frm) {
    const generate_action = values => {
        frappe.call({
            method: "gst_india.gst_india.utils.e_waybill.generate_e_waybill",
            args: {
                doctype: frm.doctype,
    ...
    const json_action = async values => {
        const ewb_data = await frappe.xcall(
            "gst_india.gst_india.utils.e_waybill.generate_e_waybill_json",
            {
                doctype: frm.doctype,
...
function show_cancel_e_waybill_dialog(frm, callback) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "gst_india.gst_india.utils.e_waybill.cancel_e_waybill",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
function show_update_vehicle_info_dialog(frm) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "gst_india.gst_india.utils.e_waybill.update_vehicle_info",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
function show_update_transporter_dialog(frm) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "gst_india.gst_india.utils.e_waybill.update_transporter",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
